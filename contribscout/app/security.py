...
def validate_remote_auth(host: str, token: str | None, *, oauth_enabled: bool = False) -> None:
    """Fail closed when HTTP is bound to a non-loopback interface."""
    is_loopback = host.lower() in {"127.0.0.1", "localhost", "::1"}
    if not is_loopback and not oauth_enabled and len((token or "").strip()) < 32:
        raise RuntimeError(
...
class McpAccessMiddleware:
    ...
    async def __call__(self, scope: dict[str, Any], receive: Any, send: Any) -> None:
        ...
        if path == "/healthz" and method in {"GET", "HEAD"}:
            ...
            return
        if self.oauth_enabled and path == "/.well-known/oauth-protected-resource" and method in {"GET", "HEAD"}:
            await self.app(scope, receive, send)
            return
        if path != "/mcp":
        ...
        if self.oauth_enabled:
            ...
            parts = authorization.split(b" ", 1)
            candidate = parts[1].strip() if len(parts) == 2 and parts[0].lower() == b"bearer" else b""
            bucket = hashlib.sha256(candidate).hexdigest() if candidate else _client_bucket(scope)
...
def _client_bucket(scope: dict[str, Any]) -> str:
